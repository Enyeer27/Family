...
app.post('/registrar', async (req, res) => {
  ...
  const { Num_Doc, nombre1, nombre2, apellido1, apellido2, tipodoc, correo, usuario, direccion, local, rol, password } = req.body;
  ...
  try {
    ...
    const queryString = 'INSERT INTO datos_personales (Id_Dato_Personal, nombre1, nombre2, apellido1, apellido2, fk_tipo_doc, correo, usuario, Direccion, Num_Local, fk_tipo_rol, password) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)';
...
app.get('/mostrar_producto', (req, res) => {
  ...
  const query = `SELECT Id_Producto, pesos.Descripcion AS pesodesc, Id_Categoria, Id_Reserva, Nombre_Producto, Cantidad, productos.Descripcion, Url_Imagen, Estado FROM productos INNER JOIN pesos ON productos.Id_Peso = pesos.Id_peso`;
  db.query(query, (error, resultado) => {
    if (error) return console.error(error.message)
    if (resultado.length > 0) {
      res.json(resultado)
